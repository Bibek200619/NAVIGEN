"""Phase 5 mock acceptance: safe startup, teleoperation, odometry, and e-stop."""
...
class TestRealBringupMockRuntime(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node.create_subscription(Odometry, '/wheel/odom', self._save_odom, 10)
        self.node.create_subscription(
    ...
    def test_safe_start_release_bounded_motion_and_estop(self):
        ...
        self.assertTrue(
            ...
            'Real bringup did not publish telemetry, odometry, joints, and static TF.',
        )
        ...
        self.assertEqual(self.odom.child_frame_id, 'base_link')
        ...
        inhibited_x = self.odom.pose.pose.position.x
        ...
        self.assertLess(
            math.fabs(self.odom.pose.pose.position.x - inhibited_x), 0.005
        )
        ...
        moving_x = self.odom.pose.pose.position.x
        self._publish_for(self.command_publisher, forward, duration=0.8)
        self.assertTrue(
            self._spin_until(
                lambda: self.telemetry.left_velocity > 0.1
                and self.odom.pose.pose.position.x > moving_x + 0.05
            ),
            'Bounded teleoperation did not produce mock encoder odometry.',
        )
        ...
        self.assertTrue(
            self._spin_until(
                ...
                and abs(self.telemetry.left_velocity) < 0.001
                and abs(self.telemetry.right_velocity) < 0.001,
                timeout=1.0,
            ),
