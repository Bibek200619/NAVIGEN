...
Works identically in simulation and on the real UGV (`/cmd_vel`).
...
ORB-SLAM3 is integrated through an adapter, never vendored. Preferred mode Stereo+IMU,
fallback Mono+IMU. Install/licensing (GPLv3) notes: `ros2_ws/src/navigen_localization/README.md`.

## 17. Nav2 Setup (Phase 3)

SmacPlanner2D + RegulatedPurePursuitController + Collision Monitor, configured in
`ros2_ws/src/navigen_navigation/`. Goals are `geometry_msgs/PoseStamped` in the local map frame.

## 18. Autonomous Demo (acceptance test)
...
| Phase | Scope | Status |
|---|---|---|
| 1 | Repo, packages, URDF, TF, config | ✅ |
| 2 | Gazebo sim + teleop | ✅ |
| 3 | Nav2 point-to-point (sim) | ⬜ |
| 4 | ESP32 firmware + serial bridge | ⬜ (protocol + bridge ready, firmware pending) |
| 5 | Real teleop | ⬜ |
| 6 | Wheel odom + IMU + EKF | ⬜ (EKF config ready) |
| 7 | Camera + perception | ⬜ (backend abstraction ready) |
| 8 | ORB-SLAM3 | ⬜ |
| 9 | Traversability → costmap | ⬜ |
| 10 | Safety supervisor integration | ⬜ (supervisor ready) |
