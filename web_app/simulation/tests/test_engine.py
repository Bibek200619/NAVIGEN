...
from engine import OBJECTS, Simulation
...
class DemoTests(unittest.TestCase):
    def test_inspection_reaches_all_checkpoints(self):
        ...
        self.assertAlmostEqual(sim.x, -7)
        self.assertAlmostEqual(sim.y, -6)
    ...
    def assert_clear(self, sim):
        self.assertTrue(
            segment_clear(
                (sim.x, sim.y),
                (sim.x, sim.y),
                rectangles(OBJECTS, sim.obstacle, ROVER_RADIUS),
                ROVER_RADIUS,
            ),
            f"Collision at {(sim.x, sim.y)}",
        )

    def test_detours_on_each_side_of_the_warehouse(self):
        for checkpoint in range(4):
            with self.subTest(checkpoint=checkpoint):
                ...
                for _ in range(3000):
                    sim.step(0.05)
                    if sim.target == checkpoint and sim.linear_velocity > 1:
                        break
    ...
    def test_tight_turn_keeps_clear_of_racks(self):
        sim = Simulation()
        sim.command("start")
        sim.target = 1
        sim.x, sim.y, sim.yaw = 7.260254, -4.152154, 1.592653
        sim.command("obstacle")
        self.assertEqual(sim.status, "avoiding")
        for _ in range(3000):
            sim.step(0.05)
    ...
    def test_unreachable_checkpoint_waits_without_collision(self):
        sim = Simulation()
        sim.command("start")
        sim.obstacle = {"x": 7, "y": -6, "radius": 0.55}
        sim.plan_route()
        self.assertEqual(sim.status, "blocked")
        for _ in range(20):
            sim.step(0.05)
        self.assertEqual((sim.x, sim.y), (-7, -6))
        sim.command("clear_obstacle")
        self.assertEqual(sim.status, "running")
