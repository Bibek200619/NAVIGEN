...
export interface OdometryStatusProps {
  sensor?: SensorStatusResponse | null;
}

export const OdometryStatus: React.FC<OdometryStatusProps> = ({ sensor }) => {
  const badge = getSensorBadgeInfo(sensor);
  const topic = sensor?.topic ?? ROS_TOPICS.WHEEL_ODOM;
  const title = sensor?.name ?? 'Wheel Odometry';
