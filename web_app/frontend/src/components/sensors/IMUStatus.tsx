...
export interface IMUStatusProps {
  sensor?: SensorStatusResponse | null;
}

export const IMUStatus: React.FC<IMUStatusProps> = ({ sensor }) => {
  const badge = getSensorBadgeInfo(sensor);
  const topic = sensor?.topic ?? ROS_TOPICS.IMU_DATA;
  const title = sensor?.name ?? 'IMU Sensor';
