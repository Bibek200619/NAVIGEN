...
export interface JointStateStatusProps {
  sensor?: SensorStatusResponse | null;
}

export const JointStateStatus: React.FC<JointStateStatusProps> = ({ sensor }) => {
  const badge = getSensorBadgeInfo(sensor);
  const topic = sensor?.topic ?? ROS_TOPICS.JOINT_STATES;
  const title = sensor?.name ?? 'Joint States';
